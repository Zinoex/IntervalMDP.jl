@testitem "getters" tags = [:base, :getters] begin
    ...
    @testset "getters" begin
        @testset "DFA reachability" begin
            ...
            prop = InfiniteTimeDFAReachability([3], 1.0e-6)
            @test !(IntervalMDP.isfinitetime(prop))
        ...
        @testset "DFA safety" begin
            ...
            prop = InfiniteTimeDFASafety([3], 1.0e-6)
            @test !(IntervalMDP.isfinitetime(prop))
            @test convergence_eps(prop) == 1.0e-6
            @test avoid(prop) == [3]
            io = IOBuffer()
        ...
        @testset "expected exit time" begin
            prop = ExpectedExitTime([3], 1.0e-6)
            @test !(IntervalMDP.isfinitetime(prop))
            @test convergence_eps(prop) == 1.0e-6
            @test avoid(prop) == [CartesianIndex(3)]
            io = IOBuffer()
...
@testitem "specification" tags = [:base, :specification] begin
    using IntervalMDP
...
@testitem "errors" tags = [:base, :errors] begin
    using IntervalMDP
    @testset "errors" begin
        ...
        tv_prod_strat = TimeVaryingStrategy([Tuple{Int32}[(1,) (1,); (1,) (1,); (1,) (1,)]])
        ...
        @testset "convergence epsilon" begin
            ...
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReachability([3], -0.001)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReachAvoid([3], [2], 0.0)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReachAvoid([3], [2], -0.001)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeSafety([3], 0.0)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeSafety([3], -0.001)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReward([1.0, 2.0, 3.0], 0.9, 0.0)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReward([1.0, 2.0, 3.0], 0.9, -0.001)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = ExpectedExitTime([3], 0.0)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = ExpectedExitTime([3], -0.001)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
        end
        @testset "infinite time property/time-varying controller" begin
            prop = InfiniteTimeDFAReachability([2], 1.0e-6)
            spec = Specification(prop)
            @test_throws ArgumentError VerificationProblem(prod_proc, spec, tv_prod_strat)
            prop = InfiniteTimeDFASafety([2], 1.0e-6)
            spec = Specification(prop)
            @test_throws ArgumentError VerificationProblem(prod_proc, spec, tv_prod_strat)
            prop = InfiniteTimeReachability([3], 1.0e-6)
            spec = Specification(prop)
        ...
        @testset "specification state errors" begin
            @testset "finite time" begin
                @testset "DFA reachability" begin
                    ...
                    prop = FiniteTimeDFAReachability([0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                end
                @testset "DFA safety" begin
                    ...
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                    prop = FiniteTimeDFASafety([0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                end
                @testset "reachability" begin
                    prop = FiniteTimeReachability([4], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeReachability([0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeReachability([(3, 2)], 10)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                end
                @testset "reach/avoid" begin
                    prop = FiniteTimeReachAvoid([4], [2], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeReachAvoid([0], [2], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeReachAvoid([2], [4], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeReachAvoid([2], [0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeReachAvoid([(3, 2)], [(2, 3)], 10)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                    prop = FiniteTimeReachAvoid([2], [2], 10)
                    spec = Specification(prop)
                    @test_throws DomainError VerificationProblem(mc, spec)
                end
                @testset "safety" begin
                    prop = FiniteTimeSafety([4], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeSafety([0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = FiniteTimeSafety([(3, 2)], 10)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                end
            end
            @testset "exact time" begin
                @testset "reachability" begin
                    prop = ExactTimeReachability([4], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExactTimeReachability([0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExactTimeReachability([(3, 2)], 10)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                end
                @testset "reach/avoid" begin
                    prop = ExactTimeReachAvoid([4], [2], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExactTimeReachAvoid([0], [2], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExactTimeReachAvoid([2], [4], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExactTimeReachAvoid([2], [0], 10)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExactTimeReachAvoid([(3, 2)], [(2, 3)], 10)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                    prop = ExactTimeReachAvoid([2], [2], 10)
                    spec = Specification(prop)
                    @test_throws DomainError VerificationProblem(mc, spec)
                end
            end
            @testset "infinite time" begin
                @testset "DFA reachability" begin
                    ...
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                    prop = InfiniteTimeDFAReachability([0], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                end
                @testset "DFA safety" begin
                    ...
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                    prop = InfiniteTimeDFASafety([0], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(prod_proc, spec)
                end
                @testset "reachability" begin
                    prop = InfiniteTimeReachability([4], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachability([0], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachability([(3, 2)], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                end
                @testset "reach/avoid" begin
                    prop = InfiniteTimeReachAvoid([4], [2], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachAvoid([0], [2], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachAvoid([2], [4], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachAvoid([(3, 2)], [(2, 3)], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachAvoid([2], [0], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeReachAvoid([2], [2], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws DomainError VerificationProblem(mc, spec)
                end
                @testset "safety" begin
                    prop = InfiniteTimeSafety([4], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeSafety([0], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = InfiniteTimeSafety([(3, 2)], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                end
                @testset "expected exit time" begin
                    prop = ExpectedExitTime([4], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExpectedExitTime([0], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws InvalidStateError VerificationProblem(mc, spec)
                    prop = ExpectedExitTime([(3, 2)], 1.0e-6)
                    spec = Specification(prop)
                    @test_throws StateDimensionMismatch VerificationProblem(mc, spec)
                end
            end
        ...
        @testset "reward discount" begin
            ...
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReward([1.0, 2.0, 3.0], 1.0, 1.0e-6)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
            prop = InfiniteTimeReward([1.0, 2.0, 3.0], 0.0, 1.0e-6)
            spec = Specification(prop)
            @test_throws DomainError VerificationProblem(mc, spec)
        end
        @testset "reward shape" begin
            ...
            prop = InfiniteTimeReward([1.0, 2.0], 0.9, 1.0e-6)
            spec = Specification(prop)
            @test_throws DimensionMismatch VerificationProblem(mc, spec)
        end
