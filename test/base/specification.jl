@testitem "base/specification: getters — DFA reachability" begin
...
@testitem "base/specification: getters — DFA safety" begin
...
@testitem "base/specification: getters — reachability" begin
...
@testitem "base/specification: getters — expected exit time" begin
...
@testitem "base/specification: specification" begin
    prop = FiniteTimeDFAReachability([3], 10)
...
@testmodule BaseSpecificationModels begin
    using IntervalMDP

    const prob = IntervalAmbiguitySets(;
        lower = [
            0.0 0.5 0.0
            0.1 0.3 0.0
            0.2 0.1 1.0
    ...
    const mc = IntervalMarkovChain(prob)
    const tv_strat = TimeVaryingStrategy([Tuple{Int32}[(1,), (1,)]])

    # Product model - just simple reachability
    const delta = TransitionFunction(Int32[
        1 2
        2 2
    ])
    const istate = Int32(1)
    const atomic_props = ["reach"]
    const dfa = DFA(delta, istate, atomic_props)

    const labelling = DeterministicLabelling(Int32[1, 1, 2])

    const prod_proc = ProductProcess(mc, dfa, labelling)
    const tv_prod_strat = TimeVaryingStrategy([Tuple{Int32}[
        (1,) (1,)
        (1,) (1,)
        (1,) (1,)
    ]])
...
@testitem "base/specification: errors — time horizon" setup =
    ...
    tv_prod_strat = BaseSpecificationModels.tv_prod_strat
...
@testitem "base/specification: errors — time horizon/time-varying strategy" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
    tv_strat = BaseSpecificationModels.tv_strat
    prod_proc = BaseSpecificationModels.prod_proc
...
@testitem "base/specification: errors — convergence epsilon" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
    prod_proc = BaseSpecificationModels.prod_proc
...
@testitem "base/specification: errors — infinite time property/time-varying controller" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
    tv_strat = BaseSpecificationModels.tv_strat
    prod_proc = BaseSpecificationModels.prod_proc
...
@testitem "base/specification: errors — specification state errors — finite time" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
    prod_proc = BaseSpecificationModels.prod_proc
...
@testitem "base/specification: errors — specification state errors — exact time" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
...
@testitem "base/specification: errors — specification state errors — infinite time" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
    prod_proc = BaseSpecificationModels.prod_proc
...
@testitem "base/specification: errors — incompatible model and specification" setup =
    [BaseSpecificationModels] begin
    mc = BaseSpecificationModels.mc
    prod_proc = BaseSpecificationModels.prod_proc
