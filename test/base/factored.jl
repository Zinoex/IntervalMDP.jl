@testitem "base/factored: invalid factored MDPs" begin
    N = Float64
    state_indices = (1, 2, 3)
    action_indices = (1,)
    state_vars = (3, 3, 3)
...
@testitem "base/factored: non-interval — 1d" begin
    N = Float64
...
@testitem "base/factored: non-interval — 2d" begin
    N = Float64
    struct SingletonAmbiguitySets <: IntervalMDP.AbstractAmbiguitySets
        transition_matrix::Matrix{N}
    end
...
@testitem "base/factored: show 1d" begin
    N = Float64
...
@testitem "base/factored: show 3d" begin
    N = Float64
    state_indices = (1, 2, 3)
    action_indices = (1,)
    state_vars = (3, 3, 3)
...
@testitem "base/factored: bellman 1d" begin
...
@testitem "base/factored: bellman 2d" begin
    @testset for N in [Float32, Float64]
        state_indices = (1, 2)
        action_indices = (1,)
        state_vars = (2, 3)
...
@testitem "base/factored: bellman 2d partial dependence" begin
    @testset for N in [Float32, Float64]
        state_vars = (2, 3)
        action_vars = (1, 2)
...
@testitem "base/factored: bellman 3d" begin
    @testset for N in [Float32, Float64]
        state_indices = (1, 2, 3)
        action_indices = (1,)
        state_vars = (3, 3, 3)
...
@testitem "base/factored: value iteration algorithms" begin
                ...
                if !isa(bellman_algorithm(alg), VertexEnumeration)
                    ...
                    for I in CartesianIndices((3, 3, 3, 3))
                        for J in CartesianIndices((3, 3, 3, 3))
                            marginal_ambiguity_sets =
                                map(marginal -> marginal[act_idx, I], marginals)
