@testitem "bellman" tags = [:cuda, :bellman] begin
    using IntervalMDP, CUDA
    if CUDA.functional()
        @testset for N in [Float32, Float64]
            prob1 = IntervalAmbiguitySets(;
                lower = N[0 1 // 2; 1 // 10 3 // 10; 1 // 5 1 // 10],
                upper = N[1 // 2 7 // 10; 3 // 5 1 // 2; 7 // 10 3 // 10],
            )
            prob2 = IntervalAmbiguitySets(;
                lower = N[1 // 10 1 // 5; 1 // 5 3 // 10; 3 // 10 2 // 5],
                upper = N[3 // 5 3 // 5; 1 // 2 1 // 2; 2 // 5 2 // 5],
            )
            prob3 =
                IntervalAmbiguitySets(; lower = N[0 0; 0 0; 1 1], upper = N[0 0; 0 0; 1 1])
...
@testitem "explicit sink state" tags = [:cuda, :explicit_sink_state] begin
    using IntervalMDP, CUDA
    if CUDA.functional()
        @testset for N in [Float32, Float64]
            prob1 = IntervalAmbiguitySets(;
            ...
            @testset "explicit sink state" begin
                ...
                @testset "infinite time reward" begin
                    prop = IntervalMDP.cu(
                        InfiniteTimeReward(N[2, 1, 0], N(9 // 10), N(1 // 1000000)),
                    )
                    spec = Specification(prop, Pessimistic, Maximize)
                    problem = VerificationProblem(mdp, spec)
                    (V_conv, _, u) = solve(problem)
    ...
    if CUDA.functional()
        @testset for N in [Float32, Float64]
            ...
            @testset "implicit sink state" begin
                ...
                implicit_mdp =
                    IntervalMDP.cu(IntervalMarkovDecisionProcess(transition_probs))
                @testset "finite time reachability" begin
                    prop = FiniteTimeReachability([3], 10)
                    spec = Specification(prop, Pessimistic, Maximize)
                    problem = VerificationProblem(mdp, spec)
                    ...
                    @test IntervalMDP.cpu(V) ≈ IntervalMDP.cpu(V_implicit) atol = 1.0e-5
                    @test k == k_implicit
                    @test IntervalMDP.cpu(res) ≈ IntervalMDP.cpu(res_implicit) atol = 1.0e-5
                    spec = Specification(prop, Optimistic, Maximize)
                    problem = VerificationProblem(mdp, spec)
                    (V, k, res) = solve(problem)
                    problem_implicit = VerificationProblem(implicit_mdp, spec)
