@testitem "cuda/sparse/vi.jl" tags = [:cuda] begin
    using IntervalMDP, CUDA, SparseArrays
    if CUDA.functional()
        @testset for N in [Float32, Float64]
            prob = IntervalAmbiguitySets(;
                lower = sparse(N[0 1 // 2 0; 1 // 10 3 // 10 0; 1 // 5 1 // 10 1]),
                upper = sparse(N[1 // 2 7 // 10 0; 3 // 5 1 // 2 0; 7 // 10 3 // 10 1]),
            )
            ...
            mc_cpu = IntervalMarkovChain(prob)
            ...
            V_fixed_it = IntervalMDP.cpu(V_fixed_it)
            @test k == 10
            ...
            @test k == k_cpu
            @test V_fixed_it ≈ V_fixed_it_cpu atol = 1.0e-5
            @test IntervalMDP.cpu(res) ≈ res_cpu atol = 1.0e-5
            ...
            (V_fixed_it_cpu, k_cpu, res_cpu) = solve(problem)
            @test k == k_cpu
            @test V_fixed_it2 ≈ V_fixed_it_cpu atol = 1.0e-5
            @test IntervalMDP.cpu(res) ≈ res_cpu atol = 1.0e-5
            ...
            @test V_fixed_it2 ≈ V_fixed_it_cpu atol = 1.0e-5
            @test IntervalMDP.cpu(res) ≈ res_cpu atol = 1.0e-5
            prop = InfiniteTimeSafety([3], N(1 // 1000000))
            spec = Specification(prop, Pessimistic)
            problem = VerificationProblem(mc, spec)
            (V_conv, _, u) = solve(problem)
            V_conv = IntervalMDP.cpu(V_conv)
            ...
            @test IntervalMDP.cpu(res) ≈ res_cpu atol = 1.0e-5
            prop =
                IntervalMDP.cu(InfiniteTimeReward(N[2, 1, 0], N(9 // 10), N(1 // 1000000)))
            spec = Specification(prop, Pessimistic)
            problem = VerificationProblem(mc, spec)
            (V_conv, _, u) = solve(problem)
            V_conv = IntervalMDP.cpu(V_conv)
            ...
            @test all(V_conv .>= N(0))
            prop =
                IntervalMDP.cu(InfiniteTimeReward(N[2, 1, -1], N(9 // 10), N(1 // 1000000)))
            spec = Specification(prop, Pessimistic)
            problem = VerificationProblem(mc, spec)
            (V_conv, _, u) = solve(problem)
            V_conv = IntervalMDP.cpu(V_conv)
