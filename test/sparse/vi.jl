@testmodule SparseViModels begin
    using IntervalMDP
    using SparseArrays
...
@testitem "sparse/vi: construction" setup = [SparseViModels] begin
    using SparseArrays

    @testset for N in [Float32, Float64, Rational{BigInt}]
        (; prob) = SparseViModels.build(N)
...
@testitem "sparse/vi: reachability" setup = [SparseViModels] begin
    using SparseArrays

    @testset for N in [Float32, Float64, Rational{BigInt}]
        (; mc) = SparseViModels.build(N)
...
@testitem "sparse/vi: reach/avoid" setup = [SparseViModels] begin
    using SparseArrays
...
@testitem "sparse/vi: safety" setup = [SparseViModels] begin
    using SparseArrays
...
@testitem "sparse/vi: reward" setup = [SparseViModels] begin
    using SparseArrays
