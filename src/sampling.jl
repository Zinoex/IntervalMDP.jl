...
[`PriorityQueueSampling.ActionUncertaintyPriority`](@ref).
...
iteration. [`PriorityQueueSampling.PrioritizedSweep`](@ref) is the
configurable member, maintaining its ranking incrementally via prioritised
sweeping's backward propagation; [`ValueFunctionOrderedSampling`](@ref) is a
simpler one that recomputes and fully re-sorts from scratch every iteration.
...
[`PriorityQueueSampling.PrioritizedSweep`](@ref)).
"""
...
###################################
#
# The concrete priority-queue algorithms live in `prioritysweeping.jl`
# (`IntervalMDP.PriorityQueueSampling.PrioritizedSweep`), split out for the same
# reason `trajectorysampling.jl` is: the configuration vocabulary is substantial
# on its own. What stays here is what the parent module dispatches on — the
# `PriorityQueueSamplingStrategy` category, `compute_priority`, and the O-max /
# predecessor primitives of Â§4a, which the trajectory sampler shares.
