...
function check_available_actions(aa::AllAvailableActions, source_dims, state_vars, action_vars)
    if any(aa.action_vars .!= action_vars)
        throw(ArgumentError("AllAvailableActions must have action_vars equal to the MDP's action_vars. Got $(aa.action_vars) vs $action_vars."))
    end
end

# List of available actions for each state
struct ListAvailableActions{N, M, V <: AbstractVector{CartesianIndex{M}}, A <: AbstractArray{V}} <: SingleTimeStepAvailableActions
    states::A
end

function check_available_actions(aa::AbstractAvailableActions, source_dims, state_vars, action_vars)
    for (d, v, s) in zip(source_dims, state_vars, size(aa.states))
        if !(d <= s <= v)
            throw(ArgumentError("The size of available actions must be between the number of source dims $d and the number of state variables $v. Got $s."))
        end
    end
    ...
    for available_actions in aa.states  # Iterate over each state's available actions
        for a in available_actions
            if !(all(1 .<= a .<= action_vars))
                throw(ArgumentError("Each action must be between 1 and the number of action variables $action_vars. Got $a."))
            end
        end
    end
end
...
struct TimeVaryingAvailableActions{A <: SingleTimeStepAvailableActions} <: AbstractAvailableActions
    actions::Vector{A}
end

function check_available_actions(aa::TimeVaryingAvailableActions, source_dims, state_vars, action_vars)
