...
const CuSparseColumnView{Tv,Ti}  = SubArray{Tv,1,CuSparseMatrixCSC{Tv,Ti},Tuple{Base.Slice{Base.OneTo{Int}},Int},false}

function SparseArrays.nnz(x::CuSparseColumnView)
    rowidx, colidx = parentindices(x)
    return length(nzrange(parent(x), colidx))
end
SparseArrays.nzrange(S::CuSparseMatrixCSC, col::Integer) = CUDA.@allowscalar(S.colPtr[col]):(CUDA.@allowscalar(S.colPtr[col+1])-1)
