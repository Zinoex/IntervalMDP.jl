...
function IntervalMDP.bellman!(
    workspace::CuDenseProductWorkspace,
    strategy_cache::IntervalMDP.AbstractStrategyCache,
    Vres,
    V,
    prob::IntervalProbabilities{Tv},
    ...
    for i in ndims(V):1
        if i == workspace.state_index
            continue
        end

        if blocks * size(V, i) >= 2^16 - 1
            break
        end
    ...
    Iblock = compute_Iblock(V, num_block_indices, workspace.state_index)
...
function compute_Iblock(V, num_block_indices, state_index)
    sizes = ntuple(num_block_indices) do i
        i += ndims(V) - num_block_indices
        if i <= state_index
            return size(V, i - one(Int32))
        else
            return size(V, i)
        end
    end
    Iblock = CartesianIndices(sizes)

    return Iblock
end

function compute_Iother(V, num_block_indices, state_index)
    sizes = ntuple(ndims(V) - num_block_indices - 1) do i
        if i < state_index
            return size(V, i)
        else
            return size(V, i + one(Int32))
    ...
    Iother = CartesianIndices(sizes)

    return Iother
end

function dense_product_bellman_kernel!(
    workspace,
    strategy_cache,
    Vres,
    V,
    prob::IntervalProbabilities{Tv},
    stateptr,
    Iblock::CartesianIndices{N1, R1},
    Iother::CartesianIndices{N2, R2},
    value_lt,
    action_reduce,
) where {Tv, N1, R1, N2, R2}
    assume(warpsize() == 32)
    nwarps = div(blockDim().x, warpsize())
    wid = fld1(threadIdx().x, warpsize())
    ...
    for iother in Iother
        other_index = (Tuple(iother)..., Tuple(iblock)...)

        Vₒ = selectotherdims(V, workspace.state_index, other_index)
        Vₒres = selectotherdims(Vres, workspace.state_index, other_index)

        # # Perform sorting
        # dense_initialize_sorting_shared_memory!(Vₒ, value, perm)
        # block_bitonic_sort!(value, perm, value_lt)

        # # O-maxmization
        # dense_product_omaximization!(action_workspace, strategy_cache, Vₒres, value, perm, prob, stateptr, action_reduce)
    end

    return nothing
end

@inline function dense_product_omaximization!(
    action_workspace,
    strategy_cache,
    Vres,
    value,
